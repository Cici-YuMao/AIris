...
const UserPhotosCard = ({ user, onLike, onNext, showActions = true }) => {
  const navigate = useNavigate();
  const [currentPhotoIndex, setCurrentPhotoIndex] = useState(0);
  const [showAllPhotos, setShowAllPhotos] = useState(false);
  // 固定图片容器高度
  const imageContainerHeight = 600;
  ...
  const handleChatClick = (e) => {
    e.stopPropagation(); // 防止事件冒泡
    const userId = user.id || user.userInfo?.id || user.userId;
    console.log('UserPhotosCard - Starting chat with user:', { userId, user });
    if (userId) {
      navigate(`/chat/${userId}/${getUserName()}`);
    } else {
      console.error('UserPhotosCard - No userId found in user object:', user);
    }
  };
  ...
  return (
    <div className="user-photos-card">
      {/* 主要照片区域 */}
      <div className="photos-container" style={{ height: `${imageContainerHeight}px` }}>
        {photos.length > 0 ? (
          <>
            {/* 主照片 */}
            <div className="main-photo-wrapper" style={{ height: `${imageContainerHeight}px` }}>
              <img
      ...
      {showActions && (
        <div className="card-actions">
          <button className="action-btn pass" onClick={handleChatClick}>
            💬 Chat
          </button>
